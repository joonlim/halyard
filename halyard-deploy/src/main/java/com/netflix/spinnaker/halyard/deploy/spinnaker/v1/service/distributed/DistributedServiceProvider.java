...
abstract public class DistributedServiceProvider<T extends Account> extends SpinnakerServiceProvider<AccountDeploymentDetails<T>> {
  public DistributedService getDeployableService(SpinnakerService.TypeAndRole typeAndRole) {
    return getDeployableService(typeAndRole, Object.class);
  }

  public <S> DistributedService<S, T> getDeployableService(SpinnakerService.TypeAndRole typeAndRole, Class<S> clazz) {
    return null; // TODO(joonlim): look through my fields using reflection (getFieldsOfType)
   ...
   */
  public List<DistributedService> getPrioritizedDistributedServices(List<SpinnakerService.TypeAndRole> serviceTypesAndRoles) {
