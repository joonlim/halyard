...
import com.netflix.spinnaker.halyard.deploy.spinnaker.v1.profile.RoscoServiceOverridesProfileFactory;
...
public class KubernetesV2RoscoService extends RoscoService implements KubernetesV2Service<RoscoService.Rosco> {
  ...
  @Autowired
  RoscoServiceOverridesProfileFactory roscoServiceOverridesProfileFactory;

  @Override
  public List<Profile> getProfiles(DeploymentConfiguration deploymentConfiguration, SpinnakerRuntimeSettings endpoints) {
    List<Profile> profiles = super.getProfiles(deploymentConfiguration, endpoints);
    if (hasServiceOverrides(deploymentConfiguration)) {
      String filename = "rosco-service-overrides.yml";
      String path = Paths.get(getConfigOutputPath(), filename).toString();
      profiles.add(roscoServiceOverridesProfileFactory.getProfile(filename, path, deploymentConfiguration, endpoints));
    }
    return profiles;
  }
